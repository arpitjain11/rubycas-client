      ...
      class Filter
        ...
        class << self
          def filter(controller)
            ...
            last_st = controller.session[:cas_last_valid_ticket] # last service ticket (if exists)

            log.info "Last Valid Ticket #{last_st}"

            if single_sign_out(controller)
              log.info "Single Sign out is true:"

              controller.send(:render, :text => "CAS Single-Sign-Out request intercepted.")
              return false
            end

            st = read_ticket(controller) # service ticket in controller
            ...
            if st
              ...
              if st.is_valid?
                if is_new_session
                  ...
                  temp_user = User.find_by_id_and_login(controller.session[client.extra_attributes_session_key][:id], controller.session[client.username_session_key])
...
#                   end
          ...
          def read_service_url(controller)
            ...
            log.debug("Params are : #{params.inspect}")
